...
use WebSockets\Service\Logger\Adapter\Message;
use WebSockets\Service\Logger\Adapter\Syslog;
use WebSockets\Service\Logger\Adapter\File;
 ...
 * @filesource /vendor/stanislav-web/zf2-websocket-server-factory/src/Service/Server.php
 */
class Server
	implements ServerInterface, MessageInterface {

	/**
	 * Socket interface
	 *
	 * @var SocketInterface $socketInstance
	 */
	 ...
	 * Console interface
	 *
	 * @var ConsoleInterface $consoleInstance
	 */
	 ...
	 * Logger interface
	 *
	 * @var  LoggerAwareInterface|LoggerInterface $loggerInstance
	 */
	 ...
	 * Client storage
	 *
	 * @var ClientStorageInterface $clientStorageInstance
	 */
	private $clientStorageInstance;
	 ...
	 *
	 * @param SocketInterface      $socketInstance
	 * @param ConsoleInterface     $consoleInstance
	 * @param ConfigInterface      $configInstance
	 * @param LoggerAwareInterface $loggerInstance
	 * @param \SplObjectStorage    $clientsStorage
	 *
	 * @throws \Exception
	 */
	public function __construct (
		SocketInterface $socketInstance,
		ConsoleInterface $consoleInstance,
		ConfigInterface $configInstance,
		LoggerAwareInterface $loggerInstance,
		ClientStorageInterface $clientStorageInstance ) {

		$this->socketInstance = $socketInstance;
		$this->consoleInstance = $consoleInstance;
		$this->configInstance = $configInstance;
		$this->clientStorageInstance = $clientStorageInstance;

		try {

			$this->setMessager ( $loggerInstance )
			     ->setLogger ( $loggerInstance )
			     ->setDebugger ( $loggerInstance );

			$this->stdOut ( "Running server..." );

			// connect and listen primary socket
			$this->createSocketConnection ();
			$this->getClientStorage()->add(new Client($this->connectionPool[0]));
			
			$this->stdOut ( sprintf ( "Listening on: %s:%d", $this->getConfig ()->getHost (), $this->getConfig ()->getPort () ), true );
			$this->stdOut ( sprintf ( "Clients: %d / %d", $this->getClientStorage()->count(), $this->getConfig ()->getMaxClients () ), true );
	...
	/**
	 * Get server clients (connections)
	 *
	 * @return ClientStorageInterface
	 */
	public function getClientStorage () {
		return $this->clientStorageInstance;
	}
	...
	public function setMessager ( LoggerAwareInterface $loggerInstance ) {
		...
		$this->loggerInstance->setLogger ( new Message() );
	...
	public function setLogger ( LoggerAwareInterface $loggerInstance ) {
		...
		if ( true === $this->getConfig ()->isLog () ) {
			...
			$this->isLog = true;
			$this->loggerInstance->setLogger ( new File( $this->getConfig ()->getLogfile () ) );
		}
	...
	public function setDebugger ( LoggerAwareInterface $loggerInstance ) {
		...
		if ( true === $this->getConfig ()->isDebug () ) {
			...
			$this->isDebug = true;
			$this->loggerInstance->setLogger ( new Syslog() );
		}
	...
	private function createSocketConnection () {
		...
		try {
			...
			if ( true === isset( $this->connectionPool[0] ) ) {
				$this->stdOut ( "Failed. Server gone away ((", true, ColorInterface::RED );
				$this->shutdown ();
			}

			// create TCP / IP stream socket connection
			$this->connectionPool[0] = $this->getSocket ()->create ();

			// setup socket connection
			$this->getSocket ()->setOption ( SOL_SOCKET, SO_REUSEADDR, 1 );

			//bind socket to configured host:port
			$this->getSocket ()->bind ( $this->getConfig ()->getHost (), $this->getConfig ()->getPort () );

			// set limit on connections throught this socket
			$this->getSocket ()->listen ( $this->getConfig ()->getMaxClients () );
	...
	public function start () {
		...
		while ( isset( $this->connectionPool[0] ) ) {

			// always send first while u start the server
			$resource = $this->getSocket()->select(1);

			if(0 < $resource) {

				foreach ( $this->connectionPool as $clientId => $socket ) {
					...
					if ( $clientId != 0 ) {
						...
						if ( false === ( $client = socket_accept ( $this->connectionPool[0] ) ) ) {
							$this->console ( $this->__errorTpl ( "socket_accept", socket_last_error (), $this->error->get ( socket_last_error ( $this->connectionPool[0] ) ) ),
							                 true );
							socket_close ( $this->connectionPool[0] );
							...
							if ( $result !== false && $this->_clientCount < $this->config['max_clients'] && ( !isset( $this->_clientIPcount[$clientIP] )
							                                                                                  || $this->_clientIPcount[$clientIP] <
							                                                                                     $this->config['max_clients_per_ip'] )
							) {
								// add new client
	...
	public function shutdown () {
		...
		foreach ( $this->getClientStorage()->getAll() as $clientId => $client ) {

			// if the client's opening handshake is complete, tell the client the server is 'going away'
			if ( $client[2] != FrameCodeInterface::SERVER_READY_STATE_CONNECTING ) {
				$this->close ( $clientId, FrameCodeInterface::SERVER_STATUS_GONE_AWAY );
			}
			$this->stdOut ( "Destroy " . $client[0], true, ColorInterface::GRAY );
			socket_close ( $client[0] );
		}

		// close the socket which listens for incoming clients
		socket_shutdown ( $this->connectionPool[0], FrameCodeInterface::SERVER_READY_STATE_CLOSING );
		socket_close ( $this->connectionPool[0] );
		$this->stdOut ( "Connection closed.", true, ColorInterface::GRAY );
	...
	public function clientTimeout () {
		...
		$time = time ();
		$this->getClientStorage()->getAll(function($client) {
...
//			$assoc_key  = $storage->getInfo(); // return, if exists, associated with cur. obj. data; else NULL
		...
		exit;
		foreach ( $this->clientStorageInstance->getClients() as $clientId => $client ) {
	...
	public function addClient ( $socket, $clientIp ) {
		...
		if ( isset( $this->_clientIPcount[$clientIp] ) ) {
			$this->_clientIPcount[$clientIp]++;
		} else {
			$this->_clientIPcount[$clientIp] = 1;
		}
		...
		$this->connectionPool[$clientId] = $socket;
	}
	...
	public function removeClient ( $client_id ) {
		...
		unset( $this->connectionPool[$client_id], $this->clients[$client_id] );
	}
	...
	public function nextClientId () {
		...
		while ( isset( $this->connectionPool[$i] ) ) {
			$i++;
		}
	...
	public function dispatchMessage ( $clientId, $opcode, $message ) {
		...
		$lastFrameBufferLength = ( $collect['messageLength'] % $collect['bufferSize'] ) != 0 ? ( $collect['messageLength'] %
		                                                                                         $collect['bufferSize'] ) : ( $collect['messageLength'] !=
		                                                                                                                      0 ? $collect['bufferSize'] : 0 );
		...
		for ( $i = 0; $i < $collect['frameCount']; $i++ ) {
			...
			// set frame bytes
			$buffer = pack ( 'n', ( ( $fin | $opcode ) << 8 ) | $payloadLength ) . $payloadLengthExtended .
			          substr ( $message, $i * $collect['bufferSize'], $collect['bufferLength'] );
	...
	private function __sendframe ( $socket, $buffer, $len ) {
		do {
			if ( false === ( $sent = @socket_send ( $socket, $buffer, $len, 0 ) ) ) {
				$this->console ( $this->__errorTpl ( "socket_send", socket_last_error (), $this->error->get ( socket_last_error ( $socket ) ) ), true );
	...
	public function close ( $clientId, $status = false ) {
		...
		// check if client ready state is already closing or closed
		if ( $this->clients[$clientId][2] == Frame::get ( 'SERVER_READY_STATE_CLOSING' ) || $this->clients[$clientId][2] == Frame::get ( 'SERVER_READY_STATE_CLOSED' ) ) {
			return true;
		}
	...
	public function stdOut ( $message, $isDebug = false, $color = null ) {
		...
		$message = mb_convert_encoding ( $message, $this->getConfig ()->getCharset () );

		if ( true === $this->isDebug && true === $isDebug ) {

			$this->loggerInstance->debug (
				$this->consoleInstance->getConsoleAdapter ()->colorize ( $message, ColorInterface::GREEN )
			);
		} else {
			$this->loggerInstance->info (
				$this->consoleInstance->getConsoleAdapter ()->colorize ( $message, $color )
			);
		}
	}
}
